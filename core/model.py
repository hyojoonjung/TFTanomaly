...
class InterpretableMultiHeadAttention(nn.Module):
    def __init__(self, config):
        ...
        # self.register_buffer("_mask", torch.triu(torch.full((config.encoder_length, config.encoder_length), float('-inf')), 1).unsqueeze(0))
...
class TemporalFusionTransformer(nn.Module):
    ...
    def forward(self, x: Dict[str, Tensor]) -> Tensor:
        ...
        # Temporal input
        # print("t_observed_tgt : {}".format(t_observed_tgt.shape)) #[example_length]
        _historical_inputs = [t_observed_tgt[:,:self.encoder_length,:]]
        ...
        # print("historical_inputs : {}".format(historical_inputs.shape)) #[encoder_length]
        # future_inputs = t_known_inp[:, self.encoder_length:]

        # Encoders
        historical_features, _ = self.history_vsn(historical_inputs)
        history, state = self.history_encoder(historical_features)
        # future_features, _ = self.future_vsn(future_inputs)
        # future, _ = self.future_encoder(future_features, state)
        torch.cuda.synchronize() # this call gives perf boost for unknown reasons

        # skip connection
        input_embedding = torch.cat([historical_features], dim=1)
        temporal_features = torch.cat([history], dim=1)
        ...
        # Static enrichment
        enriched = self.enrichment_grn(temporal_features)

        # Temporal self attention
        # print("enriched : {}".format(enriched.shape))
        x, _ = self.attention(enriched, mask_future_timesteps=True)
        # print("x : {}".format(x.shape)) #[encoder_length]
